...
$.toolbar = {
	...
	reset: function() {
		...
		$.toolbar.apply_filter($('#grayscale'), 'grayscale', false);
		$.toolbar.apply_filter($('#invert'), 'invert', false);
		$.toolbar.apply_filter($('#contrast'), 'contrast', false);
...
$.workspaceImages = {
	...
	make_images_draggable: function() {
		...
		$('.image_active').draggable(draggableOptions).children('img').addClass('img-responsive').resizable({
			aspectRatio: true,
			resize: function(event, ui) {
				if ($.select_group.imagesSelected.length <= 1) {
					$('#mini_' + ui.element.parent().attr('id')).animate({
						'width': parseInt($(this).css('width')) / 40 + "px",
						'height': parseInt($(this).css('height')) / 30 + "px",
					}, 10);
				} else {
					$.each($('.selected'), function() {
						$('#mini_' + $(this).attr('id')).animate({
							'width': parseInt($(this).children().css('width')) / 40 + "px",
		...
		this.letters_div.show().animate({
			...
			complete: function() {
				...
				$('#export_xml').unbind().click(function() {
					$.letters.export_as_xml('xml');
				});

				$('#export_html').unbind().click(function() {
					$.letters.export_as_xml('html');
				});
	...
	make_workable: function(letter) {
		...
		wrap.children().resizable({
			...
			resize: function(event, ui) {
				...
				$('#mini_' + ui.element.parent().attr('id')).animate({
					'width': parseInt(element.css('width')) / 40 + "px",
						...
						for (var j = 0; j < $.letters.regions.length; j++) {
							for (var n = 0; n < $.letters.regions[j].letters.length; n++) {
								if ($.letters.regions[j].letters[n].attr('id') == ids2[i]) {
									...
									if (!$.letters.regions[j].letters.length) {
										$.letters.to_regions();
	...
	to_workspace: function() {
		...
		if (this.lettersSelected.length) {
			for (var i = 0; i < this.lettersSelected.length; i++) {
				...
				$.letters.lettersSelected.splice(i, 1);
				i--;
			}
		} else {
			return false;
		...
		if (value == 'xml') {
			...
			$.ajax({
				...
				success: function(data) {
					...
					a.href = window.URL.createObjectURL(htmlFile);
	...
	show_manager: function() {
		$('#import').show().animate({
			...
			complete: function() {
				...
				var breadcrumb = "<div class='row-fluid'><div style='line-height:2;margin:0;' class='breadcrumb'><li><a id='back_to_load'>Load a session</a> </li><li class='active'>Local Manager</li> <li class='pull-right no-before'><button id='load_session_button' class='btn btn-primary btn-sm'>Load</button> <button id='delete_session_button' class='btn btn-danger btn-sm'>Delete</button></li></div></div>";
	...
	printImages: function(images, images_properties) {
		for (var i = 0; i < images.length; i++) {
			if (images_properties) {
				...
				var image = '<div data-size = "' + images[i][4] + '" data-title = "' + images[i][2] + '" class="image" id = "' + parseInt(images[i][1]) + '">' + images[i][0] + "<div class='col-lg-8 col-md-8 col-xs-8 offset1 image_desc'> <p><b>Manuscript</b>: " + images[i][2] + "</p> " + "<p><b>Repository</b>: " + images[i][3] + "<p><b>Place</b>: " + images[i][4] + "</p></div><br clear='all' /></div>";
			...
			if (ui.helper.hasClass('selected')) {
				$.each($('.selected'), function() {
					var image = {
						...
						'left': parseInt($(this).css('left')) / $(window).width() * 30
					};
					$("#mini_" + $(this).attr('id')).animate({
						'left': image['left'],
						'top': image['top']
				...
				var image = {
					...
					'left': parseInt(element.css('left')) / $(window).width() * 30
				};
...
function main() {
	...
	$.loadExternalImages.init('images');
	$.loadExternalImages.init('annotations');
	...
	var width;
	var height;
	var aspect_ratio = $.utils.getAspectRatio(window_width, window_height);
