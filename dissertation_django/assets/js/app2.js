        ...
        $.fn.toolbar = {
            ...
            create: function (options) {
                ...
                if ((this.exists() === false) && ($.fn.images_on_workspace().length > 0)) {
                    ...
                    structure += "<div id='topToolbar'>Tools <span title='Close toolbar' class='pull-right' id='closeToolbar'>x</span></div>";
            ...
            makeTools: function () {
                ...
                var opacity = "<div class='tool'><span class='tool_label'>Opacity</span></div>";
                opacity += "<div id='slider' class='slider'></div>";
                tools += name_image + opacity;
                var brightness = "<div class='tool'><span class='tool_label'>Brightness</span></div>";
                brightness += "<div id='slider_brightness' class='slider'></div>";
                tools += brightness;
                var comment = "<div class='tool' id='box_buttons' style='margin-top:8%;line-height:2.2;'><button id='createComment' class='btn btn-primary btn-small'><i class='icon-book'></i> Add Note</button> <button class='btn btn-primary btn-small crop_button'><i class='icon-resize-small'></i> Activate Crop</button></div><button id='crop_image' class='btn btn-small btn-warning'>Crop Image!</button></div>";
                tools += comment;
                remove = "<div class='tool'><span class='btn btn-danger' id='removeImage'><i class='icon-remove'></i> Remove Image</span></div>";
                tools += remove;
                tools += "</div>";
                return tools;
            },
                ...
                $("#slider").slider({
                    slide: function (event, ui) {
                        if ($.fn.images_on_workspace().length > 0) {
                            ...
                            element.children().children('img').css('opacity', opacity);
                        }
                    }
                });
                return false;
                    ...
                    slide: function (event, ui) {
                        if ($.fn.images_on_workspace().length > 0) {
                            if(document.body.style.webkitFilter !== undefined){
                                element.css('-webkit-filter', 'brightness(' + ui.value / 2 + '%)');
                            } else {
                                element.css('polyfilter','brightness(' + ui.value / 2 + '%)');
                            }
                        }
                    }
                });
            ...
            /*
            set_opacity_and_brightness: function(element){
                image['opacity'] = element.children().children('img').css('opacity') * 100;
                if(document.body.style.webkitFilter !== undefined){
                    if(element.css('-webkit-filter') != "none"){
                        var brightness = element.css('-webkit-filter').getNums() * 2 * 100;
                        image['brightness'] = brightness;
                    } else {
                        image['brightness'] = 200;
                    }
                } else {
                    if(typeof document.getElementById(element.attr('id')).style.polyfilterStore != "undefined"){
                        var brightness = document.getElementById(element.attr('id')).style.polyfilterStore.getNums();
                        image['brightness'] = brightness;
                    } else {
                        image['brightness'] = 200;
                    }
...
*/
            refresh: function () {
                ...
                var features = function () {
                    ...
                    //Name
                    var images = $.fn.images_on_workspace();
                    if(images.length == 1){
                        var name_image = images[0].data('title');
                        image['name'] = name_image;
                        $.fn.toolbar.set_opacity_and_brightness(images[0]);
                    } else {
                        var name_image = images.length + "Images";
                        for(i = 0; i < images.length; i++){
                            $.fn.toolbar.set_opacity_and_brightness(images[i]);
                        }
                    }
        ...
        $.fn.imagesBox = {
            ...
            buttons: function () {
                ...
                $('.image').click(function(){
                    $.fn.workspaceImages.select_image($(this));
                });
            },
                ...
                if(images.length > 0){
                    for(i = 0; i < images.length; i++){
                        ...
                        $('.image_active').click(function () {
                            $.fn.workspaceImages.select($(this));
                        });
            ...
            select_image: function(image){    
                ...
                if($.fn.imagesBox.is_selected(image)){
                    ...
                    if(images.length == 0){
                        ...
                        for(i = 0; i < images.length; i++){
                            if(image.attr('id') == $(images[i]).attr('id')){
                                $.fn.imagesBox.imagesSelected.splice(i, 1);
                                break;
                            }
                        }
                    }
                    ...
                    if(typeof image != "undefined"){
                        $.fn.workspaceImages.images_on_workspace.push(image);
                        image.data('selected', true);
                        image.children('img').css('boxShadow', '0px 0px 18px rgba(255, 246, 9, 0.94)');
                        $.fn.toolbar.init();
            ...
            get_image: function(){
                ...
                $.ajax({
                    ...
                    beforeSend: function(){
                        ...
                        if($.fn.letters.open){
                            $('#close_letters').prev().append(' <img id="loading_letter_ajax" src="/static/img/ajax-loader.gif" />')
                        }
                    },     
            ...
            show_comparison: function(data){
                ...
                $('#comparison_box').show().animate({
                        ...
                        complete: function () {
                            ...
                            $('#image_result_compared').resizable().parent().draggable({
                                zIndex:0,
                                scroll: true
                            });
                        }
        ...
        $.fn.import = {
            ...
            restoreToolbar: function(toolbar){
                $.fn.toolbar.init();
                $.fn.toolbar.create();
                $selectedImage = $('#' + toolbar['selectedImage']);
                $.fn.toolbar.refresh();
            },
