...
Lightbox.modules.core = function(app, options) {
	...
	app.core = {
		...
		selectors: {
			body: $('body'),
			html: $('html'),
			workspace1: $(app.defaults.selectors.workspace1),
			workspace2: $(app.defaults.selectors.workspace2)
		}
	...
	if (options) {
		$.extend(app.core.defaults, options);
	}
...
Lightbox.modules.toolbar = function(app, options) {
	...
	app.toolbar = {
		...
		create: function(options, show) {
			...
			if (!(this.exists()) && ($('.image_active').length) || !show) {
				...
				app.selectors.body.append('<div id="toolbar"></div>');

				this.selectors = this.selectors || {};

				this.selectors.toolbox = $('#toolbar');

				this.selectors.toolbox.dblclick(function(event) {
		...
		features: {
			...
			opacity: function() {
				var slider_opacity = self.selectors.sliders.opacity;
				slider_opacity.slider({
					disabled: true,
					slide: function(event, ui) {
						if (app.images.images_on_workspace().length) {
							var opacity = (ui.value / 100);
							$.each(app.select_group.imagesSelected, function() {
								var mini_image = $("#" + app.minimap.namespace + this.attr('id'));
			...
			brightness: function() {
				...
				var slider_brightness = self.selectors.sliders.brightness;
				...
				slider_brightness.slider({
					...
					slide: function(event, ui) {
						$.each(app.select_group.imagesSelected, function() {
							if (app.images_on_workspace().length) {
								var mini_image = $("#" + app.minimap.namespace + this.attr('id'));
			...
			rotate: function() {
				var slider_rotate = self.selectors.sliders.rotate;
			...
			apply_filter: function(button, filter, toggle) {
				...
				var imagesSelected = app.select_group.imagesSelected;
			 ...
			 */
			size: function() {
				var slider_size = self.selectors.sliders.size;
				...
				slider_size.slider({
					...
					slide: function(event, ui) {
						$.each(app.select_group.imagesSelected, function() {
							...
							$("#" + app.minimap.namespace + this.attr('id')).animate({
				...
				if (images.length == 2) {
					...
					$.ajax({
						...
						success: function(data) {
							...
							app.toolbar.selectors.body.append(img1);
							app.toolbar.selectors.body.append(img2);
							...
							app.letters.show_comparison(result);
						}
					});
			...
			selectAll: function() {
				...
				app.select_group.imagesSelected = [];
				...
				$.each(images, function() {
					...
					$(this).find('img').resizable({
						...
						resize: function(event, ui) {
							if (app.select_group.imagesSelected.length <= 1) {
								$("#" + app.minimap.namespace + ui.element.parent().attr('id')).animate({
									'width': parseInt($(this).css('width')) / app.minimap.width + "px"
								}, 10);
							} else {
								$.each($('.selected'), function() {
									$("#" + app.minimap.namespace + $(this).attr('id')).animate({
										'width': parseInt($(this).children().css('width')) / app.minimap.width + "px",
										'height': parseInt($(this).children().css('height')) / app.minimap.height + "px"
									}, 10);
								});
							}
							self.refreshSize();
					...
					app.select_group.imagesSelected.push($(this));
				...
				var create_comment_button = self.selectors.buttons.createComment;

				if (app.select_group.imagesSelected.length == 1) {
					app.toolbar.selectors.buttons.cropButton.removeClass('disabled');
					create_comment_button.removeClass('disabled').click(function() {
						var id = app.select_group.imagesSelected[0].attr('id');
						app.comments.init(false, id, false, false, false, false, false, false);
					});

					app.comments.check_notes();

				} else {
					self.selectors.buttons.cropButton.addClass('disabled');
			...
			reset: function() {
				$.each(app.select_group.imagesSelected, function() {
					...
					self.selectors.buttons.grayscale.prop('checked', false);

				});

				self.refresh();
				self.refreshSize();
				self.features.apply_filter(app.toolbar.selectors.buttons.grayscale, 'grayscale', false);
				self.features.apply_filter(app.toolbar.selectors.buttons.invert, 'invert', false);
				self.features.apply_filter(app.toolbar.selectors.buttons.contrast, 'contrast', false);
			...
			show: function() {
				var button_toolbar = this.selectors.button;

				button_toolbar.tooltip('hide').fadeOut().remove();

				this.selectors.toolbox.show().animate({
			...
			hide: function() {
				...
				if (!this.selectors.button.length) {

					buttons.prepend("<img data-toggle='tooltip' title='Show Tools Box' id='button_toolbar' src='/static/img/_tools.png' />");

				}

				var button_toolbar = self.selectors.button;

				var last_style = self.selectors.toolbox.css(['top', 'left', 'width', 'height', 'opacity']);

				self.selectors.toolbox.animate({
					position: 'absolute',
			...
			refreshSize: function() {
				var size;
				if (app.select_group.imagesSelected.length) {
			...
			refresh: function() {
				...
				if (!this.features.selectors) {
					makeSelectors();
				}

				$.each(app.select_group.imagesSelected, function(index, value) {
					...
					var features = function() {
						...
						var name_image;
						var name_image_element = this.selectors.title;
						if (app.images_on_workspace().length) {
							name_image = value.data('title');
							if (app.select_group.imagesSelected.length > 1) {

								image['name'] = "<span id='images_group'>" + app.select_group.imagesSelected.length + " images selected</span>";
								...
								$('#images_group').on('shown.bs.popover', function() {
									...
									$('.title-image-selected').hover(function() {
										...
										$("#" + app.minimap.namespace + id).animate({
									...
									$('.delete-image').click(function() {
										...
										$("#" + app.minimap.namespace + id).remove();
					...
					var image = features();
					this.selectors.sliders.opacity.slider("option", "value", image['opacity']);
					this.selectors.sliders.brightness.slider("option", "value", image['brightness']);
					this.selectors.sliders.rotate.slider("option", "value", image['rotate']);
				});
	...
	function makeTools() {
		...
		app.toolbar.toolbox.load('/static/js/tools_template.html', function() {
			...
			if (document.body.style.webkitFilter === undefined) {
				app.toolbar.selectors.buttons.contrast.remove();
			}
	...
	function buttons() {
		...
		app.toolbar.selectors.closeButton.click(function() {
			app.toolbar.hide($(this));
			app.toolbar.is_hidden = true;
		});

		app.toolbar.selectors.buttons.reset.click(function() {
			app.toolbar.reset();
		});

		app.toolbar.selectors.buttons.grayscale.click(function() {
			app.toolbar.apply_filter($(this), 'grayscale');
		});

		app.toolbar.selectors.buttons.invert.click(function() {
			app.toolbar.apply_filter($(this), 'invert');
		});

		if (document.body.style.webkitFilter !== undefined) {
			app.toolbar.selectors.buttons.contrast.click(function() {
				app.toolbar.apply_filter($(this), 'contrast');
			});
		}

		app.toolbar.selectors.buttons.align.click(function() {
			app.align.align();
		});

		app.toolbar.selectors.buttons.compare.click(function() {
			app.toolbar.compare_toolbar();
		});

		app.toolbar.selectors.buttons.remove.click(function() {

			$.each(app.select_group.imagesSelected, function() {
				$("#" + app.minimap.namespace + $(this).attr('id')).remove();
				app.toolbar.disable();
				$(this).remove();
			});
		...
		app.toolbar.selectors.buttons.crop_image.click(function(e) {
			app.crop.get_image();
		});

		app.toolbar.selectors.buttons.selectAll.click(function() {
			app.toolbar.selectAll();
		});
		...
		app.toolbar.selectors.buttons.deselectAll.click(function() {
			app.toolbar.deselectAll();
		});

		app.toolbar.selectors.buttons.createComment.click(function() {
			var id = app.select_group.imagesSelected[0].attr('id');
			app.comments.init(false, id, false, false, false, false, false, false);
		});

		app.toolbar.features.opacity();
		app.toolbar.features.brightness();
		app.toolbar.features.rotate();
		app.toolbar.features.size();
...
function Lightbox(options) {
	...
	this.init = function() {
		...
		function restore_window() {
			...
			$('#container').css('background', 'rgb(204, 204, 204');
			windows_flag = 0;
