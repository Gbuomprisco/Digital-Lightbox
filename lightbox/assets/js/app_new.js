...
function Lightbox() {
	...
	this.toolbar = {
		...
		create: function(options) {
			...
			if ((this.exists() === false) && (this.images_on_workspace().length)) {
		...
		opacity: function() {
			$("#slider").slider({
				slide: function(event, ui) {
					if (this.images_on_workspace().length) {
						var opacity = (ui.value / 100);
						this.each(this.select_group.imagesSelected, function() {
		...
		*/
		...
		size: function() {
			...
			this.toolbar.refreshSize();

		},

		clone: function() {
			var id = uniqueid();
			var images = this.select_group.imagesSelected;
			var workspace = this.workspaceImages.workspace;
			for (var i = 0; i < images.length; i++) {
				...
				this.select_group.select(new_image);
				new_image.draggable(this.workspaceImages.draggableOptions);
		...
		selectAll: function() {
			...
			this.select_group.imagesSelected = [];
			this.each(images, function() {
				...
				this.select_group.imagesSelected.push($(this));
			});
		...
		reset: function() {
			this.each(this.select_group.imagesSelected, function() {
				...
				this.toolbar.grayscale($('#grayscale'));
				this.toolbar.refresh();
			});
		...
		buttons: function() {
			...
			$('#grayscale').click(function() {
				this.toolbar.grayscale($(this));
			});

			$('#invert').click(function() {
				this.toolbar.invert($(this));
			});
		...
		refresh: function() {
			...
			this.each(this.select_group.imagesSelected, function(index, value) {
				var features = function() {
					...
					if (this.images_on_workspace().length) {
						...
						if (this.select_group.imagesSelected.length > 1) {
							image['name'] = "<span id='images_group'>" + this.select_group.imagesSelected.length + " images selected</span>";
							...
							$('#images_group').on('shown.bs.popover', function() {
								...
								$('.delete-image').click(function() {
									var id = $(this).data('image');
									$('#' + id).fadeOut().remove();
									var imagesSelected = this.select_group.imagesSelected;
									...
									$('#mini_' + id).remove();
									this.toolbar.refresh();
									if (this.select_group.imagesSelected.length) {
										this.toolbar.enable();
									} else {
										this.toolbar.disable();
									}
	...
	this.imagesBox = {
		...
		buttons: function() {
			$('#close_barLeft').click(function() {
				this.imagesBox.hide($(this));
			});

			$('#add_to_workspace').click(function() {
				this.imagesBox.to_workspace();
			});

			$('.image').click(function() {
				this.imagesBox.select_image($(this));
			});
		},
		...
		select_image: function(image) {
			...
			var images = this.imagesBox.imagesSelected;
			if (this.imagesBox.is_selected(image)) {
				...
				if (typeof image != "undefined") {
					this.imagesBox.imagesSelected.push(image);
			...
			if (images.length) {
				...
				for (var i = 0; i < images.length; i++) {
					...
					$(images[i]).dblclick(function() {
						this.select_group.select($(this));
					});

					this.minimap.add_to_minimap($(images[i]).attr('id'));

				}
				if (typeof param == "undefined") {
					this.workspaceImages.init(); // Making images draggable
				}
				this.imagesBox.imagesSelected = []; //restore the selected elements after dragged on workspace
			} else {
	...
	this.workspaceImages = {
		...
		draggableOptions: {
			...
			start: function(event, ui) {
				this.workspaceImages.beingDragged = true;
			},
			drag: function(event, ui) {
				position = $(this).offset();
				this.minimap.update_mini_map(ui);
			},
			stop: function(event, ui) {
				this.workspaceImages.beingDragged = false;
	...
	this.comments = {
		...
		buttons: function() {
			$('.removeComment').click(function() {
				var notes = this.comments.notes;
				var note = $(this).parent().parent('.comment');
				for (var i = 0; i < notes.length; i++) {
					for (var j = 0; j < notes[j].notes.length; j++) {
						if (notes[i].notes[j].id == note.data('id')) {
							this.comments.notes[i].notes.splice(j, 1);
							j--;
						}
					}
				}
				note.fadeOut().remove();
				if (!this.comments.notes.length) {
					$('#notes_alert').show().html("No notes created");
				} else {
					$('#notes_alert').hide().html('');
				}
				this.comments.update_notes();

			});


			$('.minimizeNote').click(function() {
				this.comments.minimizeNote($(this).parent().parent('.comment'));
				this.comments.check_notes();
			});
		...
		check_notes: function() {
			if (this.select_group.imagesSelected.length == 1) {
				var image = this.select_group.imagesSelected[0];
				var notes = this.comments.notes;
				...
				open_notes.click(function() {
					this.comments.open_notes(image);
				});
			}
		},
		...
		events_on_notes: function() {
			...
			$('.edit_comment_from_box').click(function() {
				this.comments.show_comment($(this));
			});
		},
					...
					for (var j = 0; j < notes[i].notes.length; j++) {
						if (notes[i].notes[j].id == note.data('id')) {
							...
							$(note).animate({
								...
								complete: function() {
									$(this).hide();
									this.comments.init(true, image, content, title, id, size, position);
								}
							});
						}
					}
		...
		openFolder_notes: function(image_note) {
			...
			this.comments.events_on_notes();
			if (this.comments.notes.length == 0) {
				$('#notes_alert').show().html("No notes created");
			} else {
				$('#notes_alert').hide().html('');
			}
				...
				for (var i = 0; i < notes.length; i++) {
					...
					if (notes[i].image != id) {
						...
						folder.dblclick(function() {
							this.comments.openFolder_notes($(this));
						});
					}
				}
			}
		...
		clean_notes_array: function() {
			...
			for (var i = 0; i < notes.length; i++) {
				if (!notes[i].notes.length) {
					if (notes[i].image == this.select_group.imagesSelected[0].attr('id')) {
						$('#open_notes').fadeOut(300).remove();
					}
					flag = true;
					$('.note_folders').data('image', notes[i].image).remove();
					this.comments.notes.splice(i, 1);
					i--;
				} else {
					flag = false;
				}
	...
	this.crop = {
		...
		show_coords: function(c) {
			this.crop.coords = [c.x, c.y, c.x2, c.y2];
		},
		...
		get_image: function() {
			...
			this.each(this.select_group.imagesSelected, function() {
				var image = this.select_group.imagesSelected[0].children().children('img');
				...
				var data = {
					...
					'height': height,
					'width': width,
					'box': JSON.stringify(this.crop.coords),
	...
	this.letters = {
			...
			letters.show().animate({
				...
				complete: function() {
					$('#close_letters').click(function() {
						this.letters.hide_letters();
						return false;
					});

					$('#compare_letters').click(function() {
						var comparison = this.letters.compare();
						this.letters.show_comparison(comparison);
						return false;
					});
					...
					$('#load_image').on('change', function(e) {
						this.letters.import_image(e);
						return false;
					});

					$('#load_xml').on('change', function(e) {
						this.letters.import_xml(e);
						return false;
					});

					$('#export_xml').click(function() {
						this.letters.export_as_xml();
						return false;
					});

					$('#add_letters').click(function() {
						this.letters.to_workspace();
						return false;
					});
				}
			}).draggable({
						...
						if (this.folderOpen) {
							...
							while (j < this.regions[i].letters.length) {
								...
								letter.click(function() {
									this.letters.selectLetter($(this));
								});
								j++;
							}
						}
		...
		make_workable: function(letter) {
			...
			this.minimap.add_to_minimap('image_' + letter.attr('id'));
			letter.remove();
		},
		...
		openFolder: function(data) {
			...
			n = 0;
			var lettersSelected = this.letters.lettersSelected;
		...
		to_regions: function() {
			...
			this.letters.folderOpen = false;
			if (this.letters.regions.length) {
				$('#export_xml').removeClass('disabled');
			} else {
				$('#export_xml').addClass('disabled');
			}
		...
		show_comparison: function(data) {
			...
			$('#image_compared_to_workspace').click(function() {
				this.letters.make_workable($('#image_result_compared'));
		...
		make_arrays: function() {
			...
			var regions = function() {
				var ids = [];
				for (var i = 0; i < this.letters.regions.length; i++) {
					for (var j = 0; j < this.letters.regions[i].letters.length; j++) {
						ids.push(this.letters.regions[i].letters[j].attr('id'));
					}
				}
				return ids;
			};
			...
			var selected = function() {
				var ids = [];
				this.each(this.letters.lettersSelected, function() {
			...
			if (this.lettersSelected.length) {
				for (var i = 0; i < ids2.length; i++) {
					for (var h = 0; h < ids.length; h++) {
						if (ids2[i] == ids[h]) {
							for (var j = 0; j < this.letters.regions.length; j++) {
								for (var n = 0; n < this.letters.regions[j].letters.length; n++) {
									if (this.letters.regions[j].letters[n].attr('id') == ids2[i]) {
										this.letters.regions[j].letters.splice(n, 1);
										n--;
										$('#' + ids2[i]).fadeOut().remove();
										if (this.letters.regions[j].letters.length === 0) {
											this.letters.to_regions();
											$("#manuscript_" + this.letters.regions[j].id).fadeOut().remove();
											this.letters.regions.splice(j, 1);
											break;
										}
									}
								}
		...
		to_workspace: function() {
			...
			if (this.lettersSelected.length) {
				for (var i = 0; i < this.lettersSelected.length; i++) {
					this.letters.make_workable(this.lettersSelected[i]);
					delete this.letters.lettersSelected[i];
				}
			} else {
				return false;
			}
			this.letters.lettersSelected.clean(undefined);
			var selected_letters_number = $('#selected_letters_number');
			selected_letters_number.html(this.lettersSelected.length + " selected");
		},
		...
		export_as_xml: function() {
			var letters = this.letters.regions;
		...
		import_image: function(e) {
			...
			reader.onload = function(ev) {
				...
				this.imagesBox.imagesSelected.push(wrap_image);
				this.imagesBox.to_workspace();
				this.imagesBox.imagesSelected = [];
			};
		...
		import_xml: function(e) {
			...
			reader.onload = function(ev) {
				...
				var xml = this.stringToXML(letters);
				var json = x2js.xml2json(xml);
				this.letters.importLetters(json.regions.manuscript);
			};
		...
		importLetters: function(array_letters) {
			...
			if (letters.hasOwnProperty(length)) {
				this.each(letters, function(index, value) {
					letter = $('<img>');
	...
	this.import = {
		...
		refreshView: function() {
			...
			folders.click(function() {
				this.import.selectItem($(this));
			});
			folders.dblclick(function() {
				this.import.loadFile($selectedItem.attr('id'));
			...
			$('#import').show().animate({
				...
				complete: function() {
					...
					$('#delete_session_button').click(function() {
						if (typeof $selectedItem != "undefined") {
							this.import.delete_session($selectedItem.attr('id'));
							$selectedItem.children('img').css('opacity', 1);
		...
		loadFile: function(file) {
			...
			this.import.parse(file_session);
			$('#import').fadeOut();
			$('#load').fadeIn();
		},
		...
		parse: function(file) {
			...
			this.reset();
			this.import.reloadImages(images, json['image_properties']);
			this.import.importComments(comments);
			this.import.importLetters(letters);
			if (toolbar) {
				this.import.restoreToolbar(toolbar);
			}
			$('html, body').animate({
		...
		reloadImages: function(images, images_properties) {
			...
			this.import.printImages(images_loaded, images_properties);
			this.import.printLetters(letters);
		},
		...
		printLetters: function(images) {
			for (var i = 0; i < images.length; i++) {
				...
				this.minimap.add_to_minimap(images[i]['image']);
			}
		},
		...
		printImages: function(images, images_properties) {
			for (var i = 0; i < images.length; i++) {
				if (images_properties) {
					...
					if (images_properties[i]['image'] == images[i][1]) {
						...
						this.minimap.add_to_minimap(images[i][1]);
			...
			if (this.imagesBox.imagesSelected.length) {
				return true;
			}
		...
		importComments: function(comments) {
			...
			for (var i = 0; i < comments.length; i++) {
				this.comments.update_notes(comments[i]);
			}
		},

		importLetters: function(letters) {
			this.letters.regions = [];
			for (var i = 0; i < letters.length; i++) {
				for (var j = 0; j < letters[i].letters.length; j++) {
					var letter = $('<img>');
					letter.attr('class', 'letter');
		...
		reset: function() {
			this.comments.notes = [];
			this.workspaceImages.imagesSelected = [];
			this.letters.lettersSelected = [];
			$('.image_active').remove();
			$('.image').remove();
			$('.letter').remove();
			this.minimap.clean_minimap();
			this.comments.clean_notes();
			this.select_group.imagesSelected = [];
			if (this.toolbar.exists()) {
				this.toolbar.toolbox.remove();
			}
		}
		...
		export_session_properties: function() {
			...
			var create_toolbar = function() {
				...
				if (this.toolbar.exists()) {
					...
					var toolbar = {
						'position': this.toolbar.toolbox.position()
					};
	...
	this.dragWindow = {
		...
		timer: 0,
		bind: function() {
			if (!this.workspaceImages.beingDragged) {
				$("body").mousemove(function(e) {
					this.dragWindow.scroll_by(e);
				});
				$("body").mousedown(function() {
					this.dragWindow.set_timer();
				});
				$("body").mouseup(function() {
					this.dragWindow.move();
				});
			}
			return false;
		},
		...
		scroll_by: function(e) {
			...
			if (this.isMouseDown) {
				window.scrollBy(0, this.dragWindow.startPosition - this.dragWindow.gesturesY);
				return false;
			}
		},
		...
		set_timer: function() {
			...
			this.timer = window.setTimeout(this.dragWindow.GetVelocity, 50);
		},
	...
	this.loadExternalImages = {
		...
		init: function() {
			...
			var request = this.get("images/", {
				'images': JSON.stringify(images)
			});

			request.done(function(data) {
				if (this.import.printImages(data)) {
					this.imagesBox.to_workspace();
					$("#hidden_div").remove();
				}
			});
		...
		parse: function(parameter) {
			var parameters = this.utils.getParameter(parameter);
			return parameters;
		}
	...
	this.init = function() {
		...
		this.imagesBox.init(); // Launch window images function
		...
		this.select_group = {
			...
			select: function(image) {
				if (image.hasClass('selected') === true) {
					...
					this.select_group.imagesSelected.push(image);
				}
				...
				if (this.imagesSelected.length == 1) {
					...
					$('#createComment').click(function() {
						var id = this.select_group.imagesSelected[0].attr('id');
						this.comments.init(false, id, false, false, false, false, false, false);
					});
					this.comments.check_notes();
			...
			$(this).animate({
				...
				if (windows_flag == 0) {
					...
					$('#barRight').click(function(e) {
						this.workspaceImages.workspace = 'barRight';
						restore_window();
						$('html, body').animate({
					...
					$('#barRight2').click(function(e) {
						this.workspaceImages.workspace = 'barRight2';
						restore_window();
						$('html, body').animate({
							scrollTop: e.clientY - 100,
